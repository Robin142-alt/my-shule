...
The shared workspace also validates the exam-series service response after the outer API envelope is removed. `/exams/series` returns `{ success, data }`, whereas report-card collection endpoints return arrays. Query selection converts the exam response to options and reports malformed responses as retryable query errors. `report-card-api-contract.test.tsx` exercises both desks with the real query hook and HTTP client, including the nested production envelope, exam filtering, invalid responses and request recovery. It reproduces the former rendering crash without replacing the query hook with array-only mocks; all five regression cases pass after the fix.
