...
- GitHub CodeQL passed for the infrastructure commit and the dropdown follow-up. Broader CI exposed pre-existing delayed-focus races in dropdown dismissal and modal opening; deterministic regressions reproduced focus moving to a hidden menu item or away from an exam-name field. Follow-ups guard deferred focus and preserve the user's selected field. All 205 tests across the nine mobile/action contract suites pass locally after both fixes. The new durable report integration and worker tests are also wired into CI after installing Redis.
