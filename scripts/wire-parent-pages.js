...
const baseDir = path.join(__dirname, '../apps/web/src/app/parent/(dashboard)');
...
export default function Parent__MODULE_CAPITALIZED__Page() {
  ...
  const { data: moduleResult, isLoading } = useSchoolQuery<any>(
    selectedChildId ? "/api/parent/__MODULE_LOWERCASE__/" + selectedChildId : null
  );
      ...
      >
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="mt-2 text-sm text-[#64748B]">There are no __MODULE_LOWERCASE__ records available yet.</p>
          </div>
        )}
      </Panel>
...
directories.forEach(dir => {
  ...
  if (fs.existsSync(filePath)) {
    const ComponentName = dir.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('');
    const ModuleName = dir.charAt(0).toUpperCase() + dir.slice(1);
    
    const content = template
      .replace(/Parent__MODULE_CAPITALIZED__Page/g, "Parent" + ComponentName + "Page")
      .replace(/__MODULE_CAPITALIZED__/g, ModuleName)
      .replace(/__MODULE_LOWERCASE__/g, dir);
