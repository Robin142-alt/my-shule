    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('SwitchChildConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('SwitchChildConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`SwitchChildConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
