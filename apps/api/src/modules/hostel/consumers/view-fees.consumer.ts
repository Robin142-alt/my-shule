...
@Injectable()
export class ViewFeesConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  readonly name = 'view-fees.execution';
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    if (event.payload.workflow_id !== 'view-fees' && event.payload.action_id !== 'view-fees') {
      return;
    }
    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('ViewFeesConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('ViewFeesConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`ViewFeesConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
