...
export class ViewChecklistConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ViewChecklistConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('ViewChecklistConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('ViewChecklistConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`ViewChecklistConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
