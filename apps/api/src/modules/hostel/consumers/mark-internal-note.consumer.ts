...
export class MarkInternalNoteConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'MarkInternalNoteConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('MarkInternalNoteConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('MarkInternalNoteConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`MarkInternalNoteConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
