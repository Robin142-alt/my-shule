    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('MarkPresentConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('MarkPresentConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`MarkPresentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
