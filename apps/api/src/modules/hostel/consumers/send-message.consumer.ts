    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('SendMessageConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('SendMessageConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`SendMessageConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
