    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('MarkLateConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('MarkLateConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`MarkLateConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
