    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('MarkAbsentConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('MarkAbsentConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`MarkAbsentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
