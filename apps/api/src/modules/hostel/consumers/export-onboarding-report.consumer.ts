...
export class ExportOnboardingReportConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ExportOnboardingReportConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (alloc) {
          ...
          if (/active|activate|approve|accept/i.test('ExportOnboardingReportConsumer')) {
            status = 'ACTIVE';
          } else if (/end|cancel|void/i.test('ExportOnboardingReportConsumer')) {
            status = 'ENDED';
          }
          if (status) {
      ...
      this.logger.error(`ExportOnboardingReportConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
