...
export class SendsToFinanceForInvoiceGenerationConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'SendsToFinanceForInvoiceGenerationConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('SendsToFinanceForInvoiceGenerationConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('SendsToFinanceForInvoiceGenerationConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('SendsToFinanceForInvoiceGenerationConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('SendsToFinanceForInvoiceGenerationConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('SendsToFinanceForInvoiceGenerationConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`SendsToFinanceForInvoiceGenerationConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
