    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('SetActiveConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('SetActiveConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('SetActiveConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('SetActiveConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('SetActiveConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`SetActiveConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
