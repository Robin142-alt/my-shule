...
export class UndoEnrolmentConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'UndoEnrolmentConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('UndoEnrolmentConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('UndoEnrolmentConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('UndoEnrolmentConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('UndoEnrolmentConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('UndoEnrolmentConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`UndoEnrolmentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
