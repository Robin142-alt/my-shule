...
export class NewTransferApplicationConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'NewTransferApplicationConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('NewTransferApplicationConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('NewTransferApplicationConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('NewTransferApplicationConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('NewTransferApplicationConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('NewTransferApplicationConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`NewTransferApplicationConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
