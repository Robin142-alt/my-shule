...
export class CreatesStudentRecordConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'CreatesStudentRecordConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('CreatesStudentRecordConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('CreatesStudentRecordConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('CreatesStudentRecordConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('CreatesStudentRecordConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('CreatesStudentRecordConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`CreatesStudentRecordConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
