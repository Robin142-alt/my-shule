    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('ArchiveConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('ArchiveConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('ArchiveConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('ArchiveConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('ArchiveConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`ArchiveConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
