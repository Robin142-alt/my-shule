...
export class PrintTransferChecklistConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintTransferChecklistConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('PrintTransferChecklistConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('PrintTransferChecklistConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('PrintTransferChecklistConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('PrintTransferChecklistConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('PrintTransferChecklistConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`PrintTransferChecklistConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
