...
export class ViewFinanceResponseConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ViewFinanceResponseConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('ViewFinanceResponseConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('ViewFinanceResponseConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('ViewFinanceResponseConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('ViewFinanceResponseConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('ViewFinanceResponseConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`ViewFinanceResponseConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
