...
export class CreateStudentRecordsConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'CreateStudentRecordsConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('CreateStudentRecordsConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('CreateStudentRecordsConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('CreateStudentRecordsConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('CreateStudentRecordsConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('CreateStudentRecordsConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`CreateStudentRecordsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
