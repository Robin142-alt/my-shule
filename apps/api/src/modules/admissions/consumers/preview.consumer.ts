    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('PreviewConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('PreviewConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('PreviewConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('PreviewConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('PreviewConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`PreviewConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
