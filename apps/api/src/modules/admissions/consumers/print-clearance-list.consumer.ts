...
export class PrintClearanceListConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintClearanceListConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('PrintClearanceListConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('PrintClearanceListConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('PrintClearanceListConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('PrintClearanceListConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('PrintClearanceListConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`PrintClearanceListConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
