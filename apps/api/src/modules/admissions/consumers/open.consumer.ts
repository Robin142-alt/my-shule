    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('OpenConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('OpenConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('OpenConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('OpenConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('OpenConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`OpenConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
