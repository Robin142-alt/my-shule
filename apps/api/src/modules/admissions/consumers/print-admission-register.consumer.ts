...
export class PrintAdmissionRegisterConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintAdmissionRegisterConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('PrintAdmissionRegisterConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('PrintAdmissionRegisterConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('PrintAdmissionRegisterConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('PrintAdmissionRegisterConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('PrintAdmissionRegisterConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`PrintAdmissionRegisterConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
