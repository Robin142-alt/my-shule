...
export class SendAdmissionLetterConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'SendAdmissionLetterConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('SendAdmissionLetterConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('SendAdmissionLetterConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('SendAdmissionLetterConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('SendAdmissionLetterConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('SendAdmissionLetterConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`SendAdmissionLetterConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
