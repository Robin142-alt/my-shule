...
export class CreateStudentProfileConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'CreateStudentProfileConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('CreateStudentProfileConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('CreateStudentProfileConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('CreateStudentProfileConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('CreateStudentProfileConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('CreateStudentProfileConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`CreateStudentProfileConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
