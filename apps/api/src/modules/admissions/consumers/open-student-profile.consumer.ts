...
export class OpenStudentProfileConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'OpenStudentProfileConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('OpenStudentProfileConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('OpenStudentProfileConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('OpenStudentProfileConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('OpenStudentProfileConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('OpenStudentProfileConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`OpenStudentProfileConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
