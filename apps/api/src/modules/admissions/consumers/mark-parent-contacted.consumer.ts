...
export class MarkParentContactedConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'MarkParentContactedConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('MarkParentContactedConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('MarkParentContactedConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('MarkParentContactedConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('MarkParentContactedConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('MarkParentContactedConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`MarkParentContactedConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
