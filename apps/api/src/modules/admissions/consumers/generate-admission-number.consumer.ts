...
export class GenerateAdmissionNumberConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'GenerateAdmissionNumberConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('GenerateAdmissionNumberConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('GenerateAdmissionNumberConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('GenerateAdmissionNumberConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('GenerateAdmissionNumberConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('GenerateAdmissionNumberConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`GenerateAdmissionNumberConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
