...
export class ExportSelectedConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ExportSelectedConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('ExportSelectedConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('ExportSelectedConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('ExportSelectedConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('ExportSelectedConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('ExportSelectedConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`ExportSelectedConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
