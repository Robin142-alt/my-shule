...
export class GenerateAdmissionNoConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'GenerateAdmissionNoConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('GenerateAdmissionNoConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('GenerateAdmissionNoConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('GenerateAdmissionNoConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('GenerateAdmissionNoConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('GenerateAdmissionNoConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`GenerateAdmissionNoConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
