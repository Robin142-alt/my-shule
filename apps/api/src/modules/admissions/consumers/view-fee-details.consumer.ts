...
export class ViewFeeDetailsConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ViewFeeDetailsConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('ViewFeeDetailsConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('ViewFeeDetailsConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('ViewFeeDetailsConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('ViewFeeDetailsConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('ViewFeeDetailsConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`ViewFeeDetailsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
