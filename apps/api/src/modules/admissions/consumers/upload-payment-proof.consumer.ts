...
export class UploadPaymentProofConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'UploadPaymentProofConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('UploadPaymentProofConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('UploadPaymentProofConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('UploadPaymentProofConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('UploadPaymentProofConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('UploadPaymentProofConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`UploadPaymentProofConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
