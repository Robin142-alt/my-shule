    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('SendReminderConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('SendReminderConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('SendReminderConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('SendReminderConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('SendReminderConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`SendReminderConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
