    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('ViewHistoryConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('ViewHistoryConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('ViewHistoryConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('ViewHistoryConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('ViewHistoryConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`ViewHistoryConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
