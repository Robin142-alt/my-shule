    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('DuplicateConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('DuplicateConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('DuplicateConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('DuplicateConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('DuplicateConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`DuplicateConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
