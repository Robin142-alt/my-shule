    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('CancelOfferConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('CancelOfferConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('CancelOfferConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('CancelOfferConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('CancelOfferConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`CancelOfferConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
