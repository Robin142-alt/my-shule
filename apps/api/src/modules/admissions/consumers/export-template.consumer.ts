...
export class ExportTemplateConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ExportTemplateConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('ExportTemplateConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('ExportTemplateConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('ExportTemplateConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('ExportTemplateConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('ExportTemplateConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`ExportTemplateConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
