...
export class RequestFinanceConfirmationConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'RequestFinanceConfirmationConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('RequestFinanceConfirmationConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('RequestFinanceConfirmationConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('RequestFinanceConfirmationConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('RequestFinanceConfirmationConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('RequestFinanceConfirmationConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`RequestFinanceConfirmationConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
