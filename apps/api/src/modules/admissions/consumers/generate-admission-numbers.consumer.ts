...
export class GenerateAdmissionNumbersConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'GenerateAdmissionNumbersConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('GenerateAdmissionNumbersConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('GenerateAdmissionNumbersConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('GenerateAdmissionNumbersConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('GenerateAdmissionNumbersConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('GenerateAdmissionNumbersConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`GenerateAdmissionNumbersConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
