    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('RejectConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('RejectConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('RejectConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('RejectConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('RejectConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`RejectConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
