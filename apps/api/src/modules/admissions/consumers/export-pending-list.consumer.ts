...
export class ExportPendingListConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ExportPendingListConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('ExportPendingListConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('ExportPendingListConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('ExportPendingListConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('ExportPendingListConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('ExportPendingListConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`ExportPendingListConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
