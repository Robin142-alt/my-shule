    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('LinksParentConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('LinksParentConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('LinksParentConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('LinksParentConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('LinksParentConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`LinksParentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
