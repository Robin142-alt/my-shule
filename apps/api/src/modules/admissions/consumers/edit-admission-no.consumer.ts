...
export class EditAdmissionNoConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'EditAdmissionNoConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('EditAdmissionNoConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('EditAdmissionNoConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('EditAdmissionNoConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('EditAdmissionNoConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('EditAdmissionNoConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`EditAdmissionNoConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
