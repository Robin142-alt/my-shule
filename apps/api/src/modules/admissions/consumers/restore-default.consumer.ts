...
export class RestoreDefaultConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'RestoreDefaultConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('RestoreDefaultConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('RestoreDefaultConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('RestoreDefaultConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('RestoreDefaultConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('RestoreDefaultConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`RestoreDefaultConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
