...
export class CreateStudentRecordConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'CreateStudentRecordConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (app) {
          ...
          if (/approve|accept/i.test('CreateStudentRecordConsumer')) {
            status = 'ACCEPTED';
          } else if (/reject|decline/i.test('CreateStudentRecordConsumer')) {
            status = 'REJECTED';
          } else if (/cancel|void/i.test('CreateStudentRecordConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (student) {
            let status: any = undefined;
            if (/approve|accept|active|activate/i.test('CreateStudentRecordConsumer')) {
              status = 'ACTIVE';
            } else if (/reject|decline|suspend/i.test('CreateStudentRecordConsumer')) {
              status = 'SUSPENDED';
            }
            if (status) {
      ...
      this.logger.error(`CreateStudentRecordConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
