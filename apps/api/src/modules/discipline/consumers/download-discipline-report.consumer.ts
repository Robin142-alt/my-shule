...
export class DownloadDisciplineReportConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'DownloadDisciplineReportConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('DownloadDisciplineReportConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('DownloadDisciplineReportConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('DownloadDisciplineReportConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('DownloadDisciplineReportConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`DownloadDisciplineReportConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
