    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('SaveIncidentConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('SaveIncidentConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('SaveIncidentConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('SaveIncidentConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`SaveIncidentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
