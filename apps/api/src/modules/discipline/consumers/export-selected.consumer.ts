    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('ExportSelectedConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('ExportSelectedConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('ExportSelectedConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('ExportSelectedConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
