    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('OpenCaseConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('OpenCaseConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('OpenCaseConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('OpenCaseConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`OpenCaseConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
