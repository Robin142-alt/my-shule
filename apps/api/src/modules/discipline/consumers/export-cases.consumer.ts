    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('ExportCasesConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('ExportCasesConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('ExportCasesConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('ExportCasesConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`ExportCasesConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
