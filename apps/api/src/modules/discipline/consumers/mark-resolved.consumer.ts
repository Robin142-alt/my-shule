    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('MarkResolvedConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('MarkResolvedConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('MarkResolvedConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('MarkResolvedConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`MarkResolvedConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
