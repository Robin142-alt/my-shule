...
@Injectable()
export class AddStatementConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  readonly name = 'add-statement.execution';
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    if (event.payload.workflow_id !== 'add-statement' && event.payload.action_id !== 'add-statement') {
      return;
    }
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('AddStatementConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('AddStatementConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('AddStatementConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('AddStatementConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`AddStatementConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
