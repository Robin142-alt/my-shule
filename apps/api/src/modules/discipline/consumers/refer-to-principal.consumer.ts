...
export class ReferToPrincipalConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ReferToPrincipalConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('ReferToPrincipalConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('ReferToPrincipalConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('ReferToPrincipalConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('ReferToPrincipalConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`ReferToPrincipalConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
