    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('MarkReviewedConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('MarkReviewedConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('MarkReviewedConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('MarkReviewedConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`MarkReviewedConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
