...
@Injectable()
export class ViewCaseConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  readonly name = 'view-case.execution';
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    if (event.payload.workflow_id !== 'view-case' && event.payload.action_id !== 'view-case') {
      return;
    }
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('ViewCaseConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('ViewCaseConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('ViewCaseConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('ViewCaseConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`ViewCaseConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
