...
@Injectable()
export class AssignSelectedConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  readonly name = 'assign-selected.execution';
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    if (event.payload.workflow_id !== 'assign-selected' && event.payload.action_id !== 'assign-selected') {
      return;
    }
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'AssignSelectedConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('AssignSelectedConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('AssignSelectedConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('AssignSelectedConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('AssignSelectedConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`AssignSelectedConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
