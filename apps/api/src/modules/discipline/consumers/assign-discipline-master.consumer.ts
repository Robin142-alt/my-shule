...
export class AssignDisciplineMasterConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'AssignDisciplineMasterConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('AssignDisciplineMasterConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('AssignDisciplineMasterConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('AssignDisciplineMasterConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('AssignDisciplineMasterConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`AssignDisciplineMasterConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
