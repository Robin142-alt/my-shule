...
export class SaveAndReferToCounsellorConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'SaveAndReferToCounsellorConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('SaveAndReferToCounsellorConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('SaveAndReferToCounsellorConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('SaveAndReferToCounsellorConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('SaveAndReferToCounsellorConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`SaveAndReferToCounsellorConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
