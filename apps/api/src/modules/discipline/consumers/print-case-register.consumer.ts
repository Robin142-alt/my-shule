...
export class PrintCaseRegisterConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintCaseRegisterConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('PrintCaseRegisterConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('PrintCaseRegisterConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('PrintCaseRegisterConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('PrintCaseRegisterConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`PrintCaseRegisterConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
