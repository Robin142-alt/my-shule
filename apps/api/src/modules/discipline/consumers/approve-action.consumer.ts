...
export class ApproveActionConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ApproveActionConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('ApproveActionConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('ApproveActionConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('ApproveActionConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('ApproveActionConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`ApproveActionConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
