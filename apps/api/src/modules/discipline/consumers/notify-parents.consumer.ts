...
export class NotifyParentsConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'NotifyParentsConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('NotifyParentsConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('NotifyParentsConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('NotifyParentsConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('NotifyParentsConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`NotifyParentsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
