...
export class CreateParentSummonsConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'CreateParentSummonsConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('CreateParentSummonsConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('CreateParentSummonsConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('CreateParentSummonsConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('CreateParentSummonsConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`CreateParentSummonsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
