...
export class OpenSeriousCasesConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'OpenSeriousCasesConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('OpenSeriousCasesConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('OpenSeriousCasesConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('OpenSeriousCasesConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('OpenSeriousCasesConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`OpenSeriousCasesConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
