...
export class ViewAuditTrailConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ViewAuditTrailConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('ViewAuditTrailConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('ViewAuditTrailConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('ViewAuditTrailConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('ViewAuditTrailConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`ViewAuditTrailConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
