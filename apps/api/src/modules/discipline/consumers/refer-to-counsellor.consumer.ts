...
export class ReferToCounsellorConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ReferToCounsellorConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('ReferToCounsellorConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('ReferToCounsellorConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('ReferToCounsellorConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('ReferToCounsellorConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`ReferToCounsellorConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
