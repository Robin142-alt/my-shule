...
export class PrintCaseFileConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintCaseFileConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('PrintCaseFileConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('PrintCaseFileConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('PrintCaseFileConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('PrintCaseFileConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`PrintCaseFileConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
