...
export class PrintSelectedConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintSelectedConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('PrintSelectedConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('PrintSelectedConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('PrintSelectedConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('PrintSelectedConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`PrintSelectedConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
