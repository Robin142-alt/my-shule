...
export class IssueWarningLetterConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'IssueWarningLetterConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('IssueWarningLetterConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('IssueWarningLetterConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('IssueWarningLetterConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('IssueWarningLetterConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`IssueWarningLetterConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
