...
export class SaveAndNotifyParentConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'SaveAndNotifyParentConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('SaveAndNotifyParentConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('SaveAndNotifyParentConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('SaveAndNotifyParentConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('SaveAndNotifyParentConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`SaveAndNotifyParentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
