...
export class SaveAndEscalateToPrincipalConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'SaveAndEscalateToPrincipalConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('SaveAndEscalateToPrincipalConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('SaveAndEscalateToPrincipalConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('SaveAndEscalateToPrincipalConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('SaveAndEscalateToPrincipalConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`SaveAndEscalateToPrincipalConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
