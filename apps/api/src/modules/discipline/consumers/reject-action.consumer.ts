    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('RejectActionConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('RejectActionConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('RejectActionConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('RejectActionConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`RejectActionConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
