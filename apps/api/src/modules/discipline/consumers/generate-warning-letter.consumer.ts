...
export class GenerateWarningLetterConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'GenerateWarningLetterConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('GenerateWarningLetterConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('GenerateWarningLetterConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('GenerateWarningLetterConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('GenerateWarningLetterConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`GenerateWarningLetterConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
