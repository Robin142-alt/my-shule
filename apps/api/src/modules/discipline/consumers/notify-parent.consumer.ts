    ...
    try {
      if (aggregate_id) {
        ...
        if (caseRecord) {
          ...
          if (/approve|accept|resolve|close/i.test('NotifyParentConsumer')) {
            status = 'CLOSED';
          } else if (/review/i.test('NotifyParentConsumer')) {
            status = 'UNDER_REVIEW';
          } else if (/refer/i.test('NotifyParentConsumer')) {
            status = 'REFERRED';
          } else if (/issue|action/i.test('NotifyParentConsumer')) {
            status = 'ACTION_TAKEN';
          }
          if (status) {
      ...
      this.logger.error(`NotifyParentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
