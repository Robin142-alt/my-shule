...
export class AssignToClassConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'AssignToClassConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('AssignToClassConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('AssignToClassConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('AssignToClassConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('AssignToClassConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`AssignToClassConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
