    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('ViewArrearsConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('ViewArrearsConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('ViewArrearsConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('ViewArrearsConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`ViewArrearsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
