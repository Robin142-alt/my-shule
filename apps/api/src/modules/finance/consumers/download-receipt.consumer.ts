...
export class DownloadReceiptConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'DownloadReceiptConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('DownloadReceiptConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('DownloadReceiptConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('DownloadReceiptConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('DownloadReceiptConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`DownloadReceiptConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
