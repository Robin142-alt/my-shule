...
export class ApproveWaiverConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ApproveWaiverConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('ApproveWaiverConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('ApproveWaiverConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('ApproveWaiverConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('ApproveWaiverConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`ApproveWaiverConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
