...
export class PrintFinanceSummaryConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintFinanceSummaryConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('PrintFinanceSummaryConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('PrintFinanceSummaryConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('PrintFinanceSummaryConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('PrintFinanceSummaryConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`PrintFinanceSummaryConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
