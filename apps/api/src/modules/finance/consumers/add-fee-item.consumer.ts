    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('AddFeeItemConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('AddFeeItemConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('AddFeeItemConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('AddFeeItemConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`AddFeeItemConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
