...
export class ExportFeeStructureConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ExportFeeStructureConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('ExportFeeStructureConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('ExportFeeStructureConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('ExportFeeStructureConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('ExportFeeStructureConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`ExportFeeStructureConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
