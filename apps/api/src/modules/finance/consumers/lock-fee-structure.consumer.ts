    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('LockFeeStructureConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('LockFeeStructureConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('LockFeeStructureConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('LockFeeStructureConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`LockFeeStructureConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
