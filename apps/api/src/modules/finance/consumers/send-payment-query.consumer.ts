...
export class SendPaymentQueryConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'SendPaymentQueryConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (inv) {
          ...
          if (/approve|accept|issue/i.test('SendPaymentQueryConsumer')) {
            status = 'ISSUED';
          } else if (/cancel|void/i.test('SendPaymentQueryConsumer')) {
            status = 'CANCELLED';
          }
          if (status) {
          ...
          if (waiver) {
            let status: any = undefined;
            if (/approve|accept/i.test('SendPaymentQueryConsumer')) {
              status = 'APPROVED';
            } else if (/reject|decline/i.test('SendPaymentQueryConsumer')) {
              status = 'REJECTED';
            }
            if (status) {
      ...
      this.logger.error(`SendPaymentQueryConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
