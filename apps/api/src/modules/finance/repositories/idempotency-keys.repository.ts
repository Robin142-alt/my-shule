...
import { IdempotencyKeyRecord, IdempotencyRequestLock, PostedFinancialTransaction } from '../finance.types';
...
export class IdempotencyKeysRepository {
  ...
  async lockRequest(input: IdempotencyRequestLock): Promise<IdempotencyKeyRecord> {
    ...
    const result = await this.databaseService.query<IdempotencyKeyRow>(
      `
  ...
  private mapRecord(row: IdempotencyKeyRow): IdempotencyKeyRecord {
    return {
      id: row.id,
