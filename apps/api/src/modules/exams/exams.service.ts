import type { ReportCardScopeQuery } from './report-card-scope';
...
export class ExamsService {
  ...
  listReportCardGenerationScopes(query: Record<string, string | undefined> = {}) {
    if (!this.isExamsOfficer()) {
      throw new ForbiddenException('Exam approval permission is required to generate report cards');
    }
    return this.repository.listReportCardGenerationScopes({ tenant_id: this.requireTenantId(),
      exam_series_id: this.optionalText(query.exam_series_id), class_section_id: this.optionalText(query.class_section_id),
      stream_name: this.optionalText(query.stream_name) });
  }
  ...
  assertReportCardScopeAccess() {
    if (!this.hasPermission('exams:read')) {
      throw new ForbiddenException('Report cards require exams:read permission');
    }
    const role = this.currentRole();
    if (!this.isExamWorkflowAdmin() && !EXAMS_MANAGER_ROLES.has(role)
      && !DEAN_APPROVAL_ROLES.has(role) && !PRINCIPAL_RELEASE_ROLES.has(role)) {
      throw new ForbiddenException('School-wide report cards require Exams Manager, Dean or Principal access');
    }
    return this.requireTenantId();
  }

  listScopedReportCards(query: Record<string, string | undefined> = {}) {
    const tenantId = this.assertReportCardScopeAccess();
    return this.repository.listScopedReportCards({ ...query, tenant_id: tenantId,
      status_in: query.status?.split(',').map(status => status.trim()).filter(Boolean),
      search: this.optionalText(query.search),
      limit: this.parsePageLimit(query.limit, 50, 200), offset: this.parsePageOffset(query.offset) });
  }

  getReportCardScopeSummary(query: Record<string, string | undefined> = {}) {
    const tenantId = this.assertReportCardScopeAccess();
    const action = query.target_action ?? query.action ?? 'submit';
    if (action !== 'export') this.assertReportCardTransitionAllowed(action);
    return this.repository.getReportCardScopeSummary({ ...query, tenant_id: tenantId, target_action: action });
  }

  getReportCardScopeHierarchy(query: Record<string, string | undefined> = {}) {
    return this.repository.getReportCardScopeHierarchy({ tenant_id: this.assertReportCardScopeAccess(),
      exam_series_id: this.optionalText(query.exam_series_id) });
  }

  async bulkTransitionReportCards(dto: ReportCardScopeQuery & { action: string; reason?: string; preview_token?: string }) {
    const action = dto.action;
    if (!REPORT_CARD_TRANSITION_ACTIONS.has(action)) throw new BadRequestException('Unsupported report-card transition');
    this.assertReportCardTransitionAllowed(action);
    const tenantId = this.requireTenantId();
    const actorUserId = this.requireUserId();
    const actorRole = this.currentRole();
    if ((action === 'recall' || action === 'unpublish') && !dto.reason?.trim()) {
      throw new BadRequestException('A correction or withdrawal reason is required');
    }
    if (!dto.preview_token) throw new BadRequestException('Preview the affected report cards before confirming');
    const result = await this.repository.bulkTransitionReportCards({ ...dto, tenant_id: tenantId,
      actor_user_id: actorUserId, actor_role: actorRole, reason: dto.reason?.trim() }, async (card, tx) => {
      await this.schoolEvents?.recordSchoolOperation({ event: {
        id: `report-card-transition-${card.id}-${card.workflow_version}`,
        type: `report_card.${action === 'unpublish' ? 'withdrawn' : action === 'submit' ? 'submitted' : action === 'approve' ? 'approved' : action === 'recall' ? 'recalled' : 'published'}`,
        module: 'exams', actorRole, title: `Report card ${action}`, body: `Report card moved to ${card.status}.`,
        entityId: card.id, payload: { report_card_id: card.id, student_id: card.student_id,
          exam_series_id: card.exam_series_id, status: card.status, workflow_version: card.workflow_version,
          reason: dto.reason ?? null },
      }, notifications: this.reportCardTransitionNotifications({ action, tenant_id: tenantId,
        report_card_id: card.id, exam_series_id: card.exam_series_id, student_id: card.student_id }) }, tx);
      if (action === 'publish' && this.eventPublisher) {
        await this.eventPublisher.publish({ event_key: `report.card.published:${card.id}:${card.workflow_version}`,
          event_name: 'report.card.published', aggregate_type: 'report_card', aggregate_id: card.id,
          payload: { tenant_id: tenantId, report_id: card.id, student_id: card.student_id,
            exam_id: card.exam_series_id, published_by_user_id: actorUserId } }, tx);
      }
    });
    return { success: result.failed_count === 0, action, total_in_scope: result.total_in_scope,
      transitioned: result.updated_count, eligible: result.eligible_count, skipped: result.skipped_count,
      failed: result.failed_count, skipped_cards: result.skipped_cards, failed_cards: result.failed_cards,
      cards: result.updated.map(card => ({ id: card.id, student_name: card.student_name,
        previous_status: card.previous_status, new_status: card.status })) };
  }
