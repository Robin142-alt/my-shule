    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('SubmitToHodConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('SubmitToHodConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('SubmitToHodConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`SubmitToHodConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
