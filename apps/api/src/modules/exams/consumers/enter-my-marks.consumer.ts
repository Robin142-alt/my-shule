    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('EnterMyMarksConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('EnterMyMarksConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('EnterMyMarksConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`EnterMyMarksConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
