    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('AddPaperConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('AddPaperConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('AddPaperConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`AddPaperConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
