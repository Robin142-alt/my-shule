...
export class AssignInvigilatorConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'AssignInvigilatorConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('AssignInvigilatorConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('AssignInvigilatorConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('AssignInvigilatorConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`AssignInvigilatorConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
