    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('LockSetupConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('LockSetupConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('LockSetupConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`LockSetupConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
