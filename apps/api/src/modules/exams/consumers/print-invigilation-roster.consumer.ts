...
export class PrintInvigilationRosterConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintInvigilationRosterConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('PrintInvigilationRosterConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('PrintInvigilationRosterConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('PrintInvigilationRosterConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`PrintInvigilationRosterConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
