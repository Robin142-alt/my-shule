    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('RefreshConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('RefreshConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('RefreshConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`RefreshConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
