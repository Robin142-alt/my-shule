...
export class ApproveResultsConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ApproveResultsConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ApproveResultsConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ApproveResultsConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ApproveResultsConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ApproveResultsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
