...
export class AddAssessmentConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'AddAssessmentConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('AddAssessmentConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('AddAssessmentConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('AddAssessmentConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`AddAssessmentConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
