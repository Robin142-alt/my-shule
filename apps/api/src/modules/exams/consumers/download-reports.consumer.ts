...
export class DownloadReportsConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'DownloadReportsConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('DownloadReportsConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('DownloadReportsConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('DownloadReportsConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`DownloadReportsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
