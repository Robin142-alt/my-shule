...
export class ReportIrregularityConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ReportIrregularityConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ReportIrregularityConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ReportIrregularityConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ReportIrregularityConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ReportIrregularityConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
