    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ViewPaperConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ViewPaperConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ViewPaperConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ViewPaperConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
