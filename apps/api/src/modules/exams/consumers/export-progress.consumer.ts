...
export class ExportProgressConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ExportProgressConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ExportProgressConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ExportProgressConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ExportProgressConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ExportProgressConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
