    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ExportSelectedConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ExportSelectedConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ExportSelectedConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
