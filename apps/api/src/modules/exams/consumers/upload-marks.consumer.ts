    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('UploadMarksConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('UploadMarksConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('UploadMarksConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`UploadMarksConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
