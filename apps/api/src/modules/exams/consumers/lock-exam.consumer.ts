    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('LockExamConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('LockExamConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('LockExamConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`LockExamConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
