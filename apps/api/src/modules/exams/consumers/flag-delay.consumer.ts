    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('FlagDelayConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('FlagDelayConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('FlagDelayConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`FlagDelayConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
