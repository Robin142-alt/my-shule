...
export class PrintInvigilationSlipConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'PrintInvigilationSlipConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('PrintInvigilationSlipConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('PrintInvigilationSlipConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('PrintInvigilationSlipConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`PrintInvigilationSlipConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
