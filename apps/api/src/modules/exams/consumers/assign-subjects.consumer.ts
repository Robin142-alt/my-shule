...
export class AssignSubjectsConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'AssignSubjectsConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('AssignSubjectsConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('AssignSubjectsConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('AssignSubjectsConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`AssignSubjectsConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
