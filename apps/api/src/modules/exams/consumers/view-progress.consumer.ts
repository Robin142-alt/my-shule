    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ViewProgressConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ViewProgressConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ViewProgressConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ViewProgressConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
