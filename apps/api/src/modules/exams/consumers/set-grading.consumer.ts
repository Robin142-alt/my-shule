    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('SetGradingConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('SetGradingConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('SetGradingConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`SetGradingConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
