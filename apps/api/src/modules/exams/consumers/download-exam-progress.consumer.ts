...
export class DownloadExamProgressConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'DownloadExamProgressConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('DownloadExamProgressConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('DownloadExamProgressConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('DownloadExamProgressConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`DownloadExamProgressConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
