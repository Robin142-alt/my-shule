...
export class ReturnForCorrectionConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ReturnForCorrectionConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ReturnForCorrectionConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ReturnForCorrectionConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ReturnForCorrectionConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ReturnForCorrectionConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
