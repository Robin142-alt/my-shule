...
export class UploadMarksCsvConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'UploadMarksCsvConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('UploadMarksCsvConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('UploadMarksCsvConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('UploadMarksCsvConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`UploadMarksCsvConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
