...
export class ReportExamIrregularityConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'ReportExamIrregularityConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ReportExamIrregularityConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ReportExamIrregularityConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ReportExamIrregularityConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ReportExamIrregularityConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
