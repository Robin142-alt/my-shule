...
export class MessageExamsManagerConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'MessageExamsManagerConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('MessageExamsManagerConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('MessageExamsManagerConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('MessageExamsManagerConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`MessageExamsManagerConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
