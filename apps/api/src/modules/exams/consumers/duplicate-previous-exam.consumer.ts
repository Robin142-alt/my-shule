...
export class DuplicatePreviousExamConsumer implements EventConsumerDescriptor<'workflow.action.completed'> {
  ...
  async handle(event: DomainEvent<'workflow.action.completed'>): Promise<void> {
    ...
    this.logger.logEvent(this.event_name, {
      consumer: 'DuplicatePreviousExamConsumer',
      tenant_id,
      aggregate_id,
      status: 'started',
    });
    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('DuplicatePreviousExamConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('DuplicatePreviousExamConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('DuplicatePreviousExamConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`DuplicatePreviousExamConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
