    ...
    try {
      if (aggregate_id) {
        ...
        if (cycle) {
          ...
          if (/active|activate/i.test('ViewLearnersConsumer')) {
            status = 'ACTIVE';
          } else if (/close|complete/i.test('ViewLearnersConsumer')) {
            status = 'CLOSED';
          } else if (/release|publish/i.test('ViewLearnersConsumer')) {
            status = 'RELEASED';
          }
          if (status) {
      ...
      this.logger.error(`ViewLearnersConsumer failed: ${error.message}`, error.stack);
      throw error;
    }
  }
}
