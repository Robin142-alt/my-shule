...
export function buildAcademicIntelligence(input: SubjectEvidence[], scope: ExamAnalyticsScope, filters: AnalyticsFilters, availableScopes: ExamAnalyticsScopeLevel[], today = new Date().toISOString().slice(0,10)) {
  ...
  if(change!==null)insightRows.push({category:change<0?'Watch':'Positive',text:`Average ${change<0?'decreased':'increased'} by ${Math.abs(change).toFixed(1)} points.`,view:'Comparisons'});
  ...
  if(riskCount)insightRows.push({category:risks[3].count?'Critical':'High Priority',text:`${riskCount} ${riskCount===1?'learner requires':'learners require'} academic follow-up.`,view:'At Risk'});
  ...
  if(riskMovement.entered_high)insightRows.push({category:'High Priority',text:`${riskMovement.entered_high} ${riskMovement.entered_high===1?'learner':'learners'} entered High or Critical risk.`,view:'At Risk'});
  if(riskMovement.left_high)insightRows.push({category:'Positive',text:`${riskMovement.left_high} ${riskMovement.left_high===1?'learner':'learners'} moved out of High or Critical risk.`,view:'Advanced'});
  const highPerformerCount=learnerRows.filter(l=>l.high_performer).length;
