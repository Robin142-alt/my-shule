...
test('subject teacher intervention uses checked learner-subject scope, derives its baseline and emits persisted workflow events',async()=>{
  const calls:Record<string,unknown>[]=[];
  let allowed=true;
  const scope:ExamAnalyticsScope={level:'assignment',role:'teacher',actor_user_id:'teacher-1'};
  const service=new ExamsService({getStore:()=>({tenant_id:'school-a',user_id:'teacher-1',role:'teacher',permissions:['academics:read','teacher:write','exams:read']})} as never,
    {getAnalytics:async()=>buildAcademicIntelligence(allowed?[evidence({average:25})]:[],scope,{page:1,page_size:25},['assignment']),
  ...
  const dto={analytics_scope:'assignment',student_id:'learner-1',class_section_id:'form-1',subject_id:'math',exam_series_id:'exam-1',owner_user_id:'teacher-1',
