...
const CERTIFICATION_AREAS: CertificationArea[] = [
  ...
  area('pilot-rollout-gate', 'Implementation 30 pilot rollout evidence gate', [
    ...
    check('rollout-gate-artifact', 'Generated rollout artifact records technical readiness while blocking live phases without evidence', 'docs/validation/implementation30-rollout-gate.md', /(?=.*Implementation 30 Pilot Rollout Gate)(?=.*Technical readiness: pass)(?=.*Rollout complete: blocked)(?=.*Phase 6)/s),
  ]),
...
export function writeImplementation30CertificationArtifact(
  ...
  writeFileSync(outputPath, renderImplementation30CertificationMarkdown(result), 'utf8');
}
