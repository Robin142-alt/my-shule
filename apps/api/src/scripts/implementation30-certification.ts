...
const CERTIFICATION_AREAS: CertificationArea[] = [
  ...
  area('exams-report-cards', 'Backend-generated and immutable report cards', [
    ...
    check('report-card-endpoint', 'Exams API exposes report-card generation for the Exams Manager and reserves publication for the Principal lifecycle', 'apps/api/src/modules/exams/exams.controller.ts', /(?=.*report-cards\/generate)(?=.*exams:write)(?=.*report-cards\/:reportCardId\/transition)(?=.*series\/:id\/publish)(?=.*exams:read)/s),
    check('report-card-generation-service', 'Report generation persists private PDFs with verification, source revisions, atomic manifests and batch status', 'apps/api/src/modules/exams/services/report-card-generation.service.ts', /(?=.*ReportCardGenerationService)(?=.*generateStudentReportCard)(?=.*createPdfReportArtifact)(?=.*saveGeneratedReportCard)(?=.*report-staging)(?=.*source_revision)(?=.*verification_code)(?=.*generateReportCardBatch)(?=.*getReportCardBatchStatus)(?=.*verifyPrintedReportCard)/s),
    ...
    check('report-card-batch-and-verification-endpoints', 'Exams API exposes Exams Manager regeneration and batching plus read-scoped status and printed verification endpoints', 'apps/api/src/modules/exams/exams.controller.ts', /(?=.*report-cards\/regenerate)(?=.*report-cards\/batches)(?=.*report-cards\/batches\/:batchId)(?=.*report-cards\/verify\/:verificationCode)(?=.*exams:write)(?=.*exams:read)/s),
    check('immutable-published-report-cards', 'Published report cards remain immutable while controlled replacements use current and supersedes revision pointers', 'apps/api/src/modules/exams/repositories/exams.repository.ts', /createReportCardSnapshot[\s\S]+current_card[\s\S]+card\.is_current = TRUE[\s\S]+current_card\.status <> 'published'[\s\S]+revision_number[\s\S]+supersedes_report_card_id[\s\S]+Published report cards are immutable; generate a controlled revision instead/),
    ...
    check('report-card-generation-tests', 'Exams tests cover PDF artifact generation, class batch progress, and printed verification lookup', 'apps/api/src/modules/exams/exams.test.ts', /(?=.*ReportCardGenerationService generates PDF artifacts with a verification code)(?=.*starts class report-card batches)(?=.*verifies printed report cards)/s),
  ]),
