...
const CERTIFICATION_AREAS: CertificationArea[] = [
  area('principal-executive-dashboard', 'Module-aware principal executive dashboard', [
    ...
    check('executive-ui', 'School UI renders the executive dashboard and module alerts', 'apps/web/src/components/school/principal-command-center.tsx', /(?=.*PrincipalCommandCenter)(?=.*Alerts and risk center)(?=.*enabled_modules)/s),
    check('executive-ui-stream', 'School UI subscribes to principal dashboard event updates', 'apps/web/src/components/school/principal-command-center.tsx', /new EventSource[\s\S]+principal\.dashboard/),
