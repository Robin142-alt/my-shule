...
import { GameDayEventsTestModule } from './support/gameday-events-test.module';
...
describe('GameDay resilience exercise', () => {
  let pool: Pool;
  ...
  let eventsQueueService: CapturingQueueService;
  ...
  beforeAll(async () => {
    ...
    eventsTestingModule = await Test.createTestingModule({
      imports: [GameDayEventsTestModule],
    }).compile();
    ...
    eventsQueueService = eventsTestingModule.get(CapturingQueueService);
  ...
  afterEach(async () => {
    authRedis = authTestingModule.get(REDIS_CLIENT);
    mpesaMockServer?.reset();
    mpesaQueueService?.clear();
    eventsQueueService?.clear();
  ...
  test('recovers automatically after Redis becomes unavailable without leaking partial auth state', async () => {
    const tenantId = registerTenantId('gameday-auth');
  ...
  test('recovers automatically from a PostgreSQL write failure without data loss or manual cleanup', async () => {
    const fixture = await createLedgerFixture('gameday-finance');
    const idempotencyKey = `gameday-finance:${fixture.tenant_id}`;
    ...
    await expect(
      runInFinanceTenantContext(fixture.tenant_id, () =>
        transactionService.postTransaction({
          ...
          reference: `GAMEDAY-FIN-${fixture.tenant_id}`,
          description: 'GameDay PostgreSQL outage simulation',
          entries: [
            {
  ...
  test('recovers automatically from a queue worker crash on the next dispatch cycle with no duplicate effects', async () => {
    const tenantId = registerTenantId('gameday-events');
    ...
    await runInEventsTenantContext(tenantId, async () => {
      await eventsDatabaseService.withRequestTransaction(async () => {
        await eventPublisherService.publishStudentCreated({
          ...
          admission_number: `GAMEDAY-${studentId.slice(0, 8)}`,
          first_name: 'GameDay',
          last_name: 'Student',
          metadata: {
            source: 'gameday.integration-spec',
          },
        });
      });
    });
    ...
    const failedState = await getOutboxEventState(pool, tenantId);

    expect(failedState.status).toBe('failed');
    expect(failedState.attempt_count).toBe(1);
    ...
    const publishedState = await getOutboxEventState(pool, tenantId);

    expect(publishedState.status).toBe('published');
    expect(publishedState.attempt_count).toBe(2);
  ...
  test('recovers automatically after the MPESA API returns without orphaned payment state', async () => {
    ...
    await expect(
      runInMpesaTenantContext(tenantId, () =>
        mpesaService.createPaymentIntent({
          ...
          transaction_desc: 'GameDay MPESA downtime simulation',
    ...
    mpesaMockServer.enqueueScenario({
      ...
      callbacks: [{ delivery_id: `delivery-${tenantId}` }],
    });
  ...
  const runInFinanceTenantContext = async <T>(
    tenantId: string,
    callback: () => Promise<T>,
    requestId = `gameday-finance:${randomUUID()}`,
  ): Promise<T> =>
    financeRequestContext.run(
      buildRequestContext(tenantId, requestId, '/integration/gameday/finance', 'TEST'),
      callback,
    );
  ...
  const runInEventsTenantContext = async <T>(
    tenantId: string,
    callback: () => Promise<T>,
    requestId = `gameday-events:${randomUUID()}`,
  ): Promise<T> =>
    eventsRequestContext.run(
      buildRequestContext(tenantId, requestId, '/integration/gameday/events', 'TEST'),
      callback,
    );
  ...
  const runInMpesaTenantContext = async <T>(
    tenantId: string,
    callback: () => Promise<T>,
    requestId = `gameday-mpesa:${randomUUID()}`,
  ): Promise<T> =>
    mpesaRequestContext.run(
      buildRequestContext(tenantId, requestId, '/integration/gameday/mpesa', 'POST'),
      callback,
    );
    ...
    eventsRequestContext.run(
      {
        ...buildRequestContext(
          tenantId,
          `gameday-claim:${randomUUID()}`,
          '/internal/gameday/events/claim',
          'WORKER',
        ),
        role: 'system',
      },
      async () =>
        eventsDatabaseService.withRequestTransaction(async () => {
          const claimedEventResult = await eventsDatabaseService.query<{
            id: string;
            tenant_id: string;
            request_id: string;
          }>(
            `
              WITH candidate_event AS (
                ...
                FROM outbox_events
                WHERE outbox_events.tenant_id = $1
                  AND (
                    (
                    ...
                    OR (
                      ...
                      AND outbox_events.updated_at <= NOW() - (100 * INTERVAL '1 millisecond')
                    )
                  )
              ...
              WHERE target.id = candidate_event.id
              RETURNING
                target.id,
                target.tenant_id,
                COALESCE(
          ...
          const claimedEvent = claimedEventResult.rows[0];
          ...
          return {
            ...
            tenant_id: claimedEvent.tenant_id,
...
const ensureBaseIntegrationEnv = (): void => {
  ...
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required for GameDay integration tests');
  }
};
...
const buildRequestContext = (
  ...
  client_ip: '127.0.0.1',
  user_agent: 'gameday-integration-tests',
  method,
  path,
