...
export async function POST(request: NextRequest) {
  ...
  if (!tenantId || !accessToken) {
    return NextResponse.json(
      {
        synced: false,
        message: "Storekeeper session expired. Sign in again to submit the stock receipt.",
      },
      { status: 401 },
    );
  }
  ...
  if (!isDashboardApiConfigured()) {
    return NextResponse.json(
      {
        synced: false,
        message: "Live inventory API is unavailable. The stock receipt was not submitted.",
      },
      { status: 503 },
    );
  }
  ...
  try {
    const upstream = await requestDashboardApi("/inventory/stock-receipts", {
      method: "POST",
      tenantId,
      accessToken,
    ...
    return NextResponse.json({
      synced: true,
      message: "Stock receipt synced to the live inventory API.",
      upstream,
    });
  } catch (error) {
    return NextResponse.json(
