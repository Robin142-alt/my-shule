...
export async function POST(request: NextRequest) {
  ...
  if (!tenantId || !accessToken) {
    return NextResponse.json(
      { synced: false, message: "Librarian session expired. Sign in again to issue the book." },
      { status: 401 },
    );
  }
  ...
  try {
    const upstream = await requestDashboardApi("/library/circulation/issue", {
      method: "POST",
      tenantId,
      accessToken,
    ...
    return NextResponse.json({ synced: true, message: "Book issued from scanner.", upstream });
  } catch (error) {
    return NextResponse.json(
      { synced: false, message: error instanceof Error ? error.message : "Scanner issue failed." },
      { status: 502 },
    );
  }
}
