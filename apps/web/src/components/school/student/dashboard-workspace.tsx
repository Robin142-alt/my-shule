...
export function DashboardWorkspace() {
  const { data, isLoading } = useSchoolQuery<DashboardData>("/admin-command/student/dashboard");
  ...
  return (
    <section className="rounded-2xl border border-[#D8E0EC] bg-white p-5 shadow-[0_18px_50px_rgba(7,29,73,0.08)]">
      <div className="mb-4 flex min-w-0 gap-3">
        ...
        <div>
          <h2 className="text-xl font-black tracking-[-0.01em] text-[#071D49]">Student Dashboard</h2>
          <p className="mt-1 text-sm leading-6 text-[#64748B]">Overview of your academic activities.</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        ...
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4">
          <div className="text-sm font-semibold text-[#64748B]">Attendance %</div>
          <div className="mt-1 text-lg font-black text-[#071D49]">{isLoading ? "..." : data?.metrics?.attendance ?? 0}</div>
        </div>
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4">
            ...
            {isLoading ? (
              ...
              items.map((row: any, i: number) => (
                <tr key={row.id || i} className="border-t border-[#D8E0EC] hover:bg-[#F8FAFC]">
                  ...
                  <td className="px-4 py-3 text-[#64748B]">{row.grade}</td>
