...
export function MessagesWorkspace() {
  const { data, isLoading } = useSchoolQuery<MessagesData>("/admin-command/student/messages");
  ...
  return (
    <section className="rounded-2xl border border-[#D8E0EC] bg-white p-5 shadow-[0_18px_50px_rgba(7,29,73,0.08)]">
      <div className="mb-4 flex min-w-0 gap-3">
        ...
        <div>
          <h2 className="text-xl font-black tracking-[-0.01em] text-[#071D49]">Messages</h2>
          <p className="mt-1 text-sm leading-6 text-[#64748B]">View messages and announcements.</p>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              items.map((row: any, i: number) => (
                <tr key={row.id || i} className="border-t border-[#D8E0EC] hover:bg-[#F8FAFC]">
                  <td className="px-4 py-3 text-[#64748B]">{row.from}</td>
