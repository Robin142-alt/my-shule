...
export function DownloadsWorkspace() {
  const { data, isLoading, error } = useSchoolQuery<DownloadsData>("/admin-command/student/downloads");
    ...
    <section className="rounded-2xl border border-[#D8E0EC] bg-white p-5 shadow-[0_18px_50px_rgba(7,29,73,0.08)]">
      <div className="mb-4 flex min-w-0 gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-[#64748B]">Download your report cards and documents.</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm text-left whitespace-nowrap">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[#64748B]">No report cards have been published to your student account yet.</td></tr>
            ) : (
              items.map((row) => (
