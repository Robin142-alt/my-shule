  ...
  return (
    <Panel title="Academic Progress" description="Monitor the class's overall academic performance and identify learners needing support." icon={TrendingUp}>
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4 font-bold text-[#071D49]">Class Mean: {safeData.classMean}</div>
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4 font-bold text-[#071D49]">Class Grade: {safeData.classGrade}</div>
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 font-bold text-rose-700">Missing Marks: {safeData.missingMarksSubjects} subjects</div>
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 font-bold text-emerald-800 md:col-span-3">Top Performer: {safeData.topPerformer}</div>
      </div>
      <div className="mt-4 p-4 rounded bg-amber-50 text-amber-800 font-medium">
        {safeData.learnersBelowTarget} learners are performing below their target grade.
      </div>
    </Panel>
  );
}
