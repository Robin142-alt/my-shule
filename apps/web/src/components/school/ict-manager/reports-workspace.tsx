...
export function ReportsWorkspace() {
  ...
  const { data, error, isLoading, refetch } = useSchoolQuery<ReportsData>('/admin-command/ict-manager/reports');
  const generateReport = useSchoolMutation<unknown, { title: string; format: string }>('/admin-command/ict-manager/reports/generate', 'POST', {
    onSuccess: async () => { toast.success('ICT report generated.'); await refetch(); },
    onError: (mutationError) => toast.error('ICT report generation failed', { description: mutationError.message }),
  });
  const downloadReport = useSchoolMutation<ReportArtifactResponse, { id: string }>(
    ({ id }) => `/admin-command/ict-manager/reports/${id}/download`,
    'POST',
    {
      onSuccess: ({ report }) => {
        if (!report.artifact?.content_base64) {
          toast.error('ICT report download failed', { description: 'The stored report has no downloadable artifact.' });
          return;
        }
        try {
          downloadBase64File({
            filename: report.artifact.filename || `ict-report-${Date.now()}.${report.format || 'pdf'}`,
            mimeType: report.artifact.content_type || 'application/octet-stream',
            contentBase64: report.artifact.content_base64,
          });
          toast.success('ICT report downloaded.');
        } catch (downloadError) {
          toast.error('ICT report download failed', {
            description: downloadError instanceof Error ? downloadError.message : 'The artifact could not be decoded.',
          });
        }
      },
      onError: (mutationError) => toast.error('ICT report download failed', { description: mutationError.message }),
    },
  );
  ...
  if (error) {
    return (
      <Panel title="ICT Reports" description="Generate and download ICT reports." icon={FileText}>
        <WorkspaceQueryFailure title="ICT reports could not be loaded." error={error} onRetry={() => void refetch()} />
      </Panel>
    );
  }
  ...
  return (
    <Panel
      title="ICT Reports"
      description="Generate and download ICT reports."
      icon={FileText}
      actions={<button type="button" disabled={permissionsLoading || !canWrite || generateReport.isPending} onClick={() => generateReport.mutate({ title: 'ICT operations report', format: 'pdf' })} className="rounded-lg bg-[#071D49] px-4 py-2 text-sm font-black text-white disabled:opacity-50">{generateReport.isPending ? 'Generating…' : 'Generate PDF'}</button>}
    >
        ...
        <RecordTable className="w-full text-sm text-left whitespace-nowrap">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[#64748B]">No ICT report has been generated for this school. Generate the first report from live asset, loan, repair, and facility records.</td></tr>
            ) : (
              items.map(row => (
