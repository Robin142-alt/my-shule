...
export function ReportsWorkspace() {
  ...
  const handleConfirmGenerate = async () => {
    ...
    try {
      ...
      toast.success("Store report request queued. Refreshing the report list for provider status.");
  ...
  const downloadStoreReport = (report: StoreReport) => {
    downloadCsvFile({
      filename: `store-report-${report.id}.csv`,
      headers: ["Report ID", "Title", "Type", "Period", "Generated By", "Generated Date", "Status"],
      rows: [[report.id, report.title, report.type, report.period, report.generated_by, report.generated_date, report.status]],
    });
    toast.success(`${report.title} downloaded as CSV.`);
  };

  const printStoreReport = (report: StoreReport) => {
    openPrintDocument({
      eyebrow: "Store report",
      title: report.title,
      subtitle: `${report.type} - ${report.period}`,
      rows: [
        { label: "Generated by", value: report.generated_by },
        { label: "Generated date", value: report.generated_date },
        { label: "Status", value: report.status },
      ],
      footer: "Generated from the current school's store report register.",
    });
    toast.success(`${report.title} print preview ready.`);
  };
                  ...
                  <td className="px-4 py-3 text-right">
                    {report.status === "Ready" ? (
                      <div className="inline-flex gap-2">
                        <button onClick={() => downloadStoreReport(report)} className="inline-flex items-center gap-1 text-blue-600 hover:underline font-semibold text-xs">
                          <Download className="h-3 w-3" /> Download
                        </button>
                        <button onClick={() => printStoreReport(report)} className="inline-flex items-center gap-1 text-[#64748B] hover:underline font-semibold text-xs">
