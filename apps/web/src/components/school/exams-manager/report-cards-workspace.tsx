...
type ReportCardsData = {
  ...
  reportcardsList: ReportCardsRecord[];
};

export function ReportCardsWorkspace() {
  const { data, isLoading } = useSchoolQuery<ReportCardsData>('/admin-command/exams-manager/report-cards');
  const items = data?.reportcardsList || [];
  ...
  return (
    <Panel title="Report Cards" description="Generate and manage student report cards." icon={FileText}>
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4">
          <div className="text-sm font-semibold text-[#64748B]">Generated</div>
          <div className="mt-1 text-lg font-black text-[#071D49]">{isLoading ? "..." : data?.metrics?.generated ?? 0}</div>
        </div>
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4">
        ...
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4">
          <div className="text-sm font-semibold text-[#64748B]">Downloaded</div>
          <div className="mt-1 text-lg font-black text-[#071D49]">{isLoading ? "..." : data?.metrics?.downloaded ?? 0}</div>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              items.map(row => (
                <tr key={row.id} className="border-t border-[#D8E0EC] hover:bg-[#F8FAFC]">
                  ...
                  <td className="px-4 py-3 text-[#64748B]">{row.position}</td>
