...
type ClassSubjectAssignment = LifecycleRecord & { academic_term_id?: string | null; stream_id?: string | null; stream_name?: string | null; cohort_id?: string | null; cohort_name?: string | null; academic_term_name?: string; class_section_id: string; class_section_name?: string; subject_id: string; subject_name?: string; is_compulsory?: boolean; is_examinable?: boolean; effective_from?: string | null; effective_to?: string | null; reason?: string | null };
...
type AcademicFoundationResponse = {
  migrationIssues?: Array<{id: string; class_name: string; stream_name?: string; subject_name?: string; entity_type: string}>;
  years: AcademicYear[];
  terms: AcademicTerm[];
...
export function AcademicFoundationWorkspace({
  ...
  const canPromote = permissionAllows(promotionPermissions, "admissions:write") && permissionAllows(promotionPermissions, "students:write");
  ...
  const handleAssignClassSubject = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const saved = await submit("class-subject", "/academics/class-subjects/bulk", {
      ...
      is_examinable: data.get("is_examinable") === "on",
      reason: value(data, "reason") || undefined,
    }, `${subjectIds.length} ${subjectIds.length === 1 ? "subject" : "subjects"} assigned to the cohort until changed.`, form);
    if (saved) { setSelectedClassSubjectIds([]); setSubjectClassId(""); }
  };
  ...
  return (
    <section aria-label="Academic foundation setup" className="space-y-5 text-white">
      ...
      {foundationQuery.data?.migrationIssues?.length ? <section role="alert" className="rounded-xl border border-amber-300/50 bg-amber-950/40 p-4 text-amber-100">
        ...
        <p className="mt-1 text-sm">Some older assignments conflict. Assign the intended subject or teacher for each listed class and stream below. Existing records are preserved, and promotion waits for these choices.</p>
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            ...
            <SetupForm title="Create stream" description="Streams belong to one class/form/grade and are selectable in admissions and attendance.">
              <form onSubmit={handleCreateStream} className="grid gap-3 sm:grid-cols-2">
                <label className="sm:col-span-2 text-sm font-bold">Class/form/grade<select name="class_section_id" required className={fieldClass} defaultValue=""><option value="">Select class</option>{activeClasses.map((item) => <option key={item.id} value={item.id}>{item.name}{item.academic_year_id ? ` - ${labels.years.get(item.academic_year_id) || ""}` : ""}</option>)}</select></label>
        ...
        <div role="tabpanel" className="space-y-5">
          ...
          <SetupForm title="Assign subjects to cohort" description="Subjects continue with the learners across terms and annual promotion until changed. Choose one stream, or apply the setup to every current stream in the class.">
            <form aria-label="Assign subjects to cohort" onSubmit={handleAssignClassSubject} className="grid gap-3 md:grid-cols-2 xl:grid-cols-3 xl:items-end">
              ...
              <label className="text-sm font-bold">Stream<select key={subjectClassId} name="stream_id" defaultValue="" disabled={!subjectClassId} className={fieldClass}><option value="">All current streams / no stream</option>{activeStreams.filter((stream) => stream.class_section_id === subjectClassId).map((stream) => <option key={stream.id} value={stream.id}>{stream.name}</option>)}</select></label>
              ...
              <div className="flex flex-wrap gap-4 md:col-span-2 xl:col-span-3"><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" name="is_compulsory" defaultChecked /> Compulsory for this class</label><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" name="is_examinable" defaultChecked /> Examinable</label></div>
              {(activeClasses.length === 0 || activeSubjects.length === 0) ? <p className="text-xs font-bold text-amber-200 md:col-span-2 xl:col-span-3">Create an active class and subject before assigning the offering.</p> : null}
      ...
      {!isLoading && activeTab === "allocations" ? (
        <div role="tabpanel" className="space-y-5">
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            ...
            <SetupForm title="Assign subject teacher" description="The teacher continues across terms and follows the cohort as students are promoted, until reassigned or ended.">
              <form aria-label="Assign subject teacher" onSubmit={handleAssignSubjectTeacher} className="space-y-3">
                <label className="block text-sm font-bold">Class/form/grade<select name="class_section_id" required className={fieldClass} value={teacherClassId} onChange={(event) => setTeacherClassId(event.target.value)}><option value="">Select class</option>{activeClasses.map((item) => <option key={item.id} value={item.id}>{item.name}{item.academic_year_id ? ` - ${labels.years.get(item.academic_year_id) || ""}` : ""}</option>)}</select></label>
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            <SetupForm title="Assign academic leadership role" description="Assign a staff member to an academic responsibility. Reassignment ends the previous holder without erasing history.">
              <form onSubmit={handleAssignAcademicRole} className="grid gap-3 sm:grid-cols-2">
                ...
                <label className="text-sm font-bold">Class scope<select name="class_section_id" defaultValue="" className={fieldClass}><option value="">Not applicable</option>{activeClasses.map((item) => <option key={item.id} value={item.id}>{item.name}{item.academic_year_id ? ` - ${labels.years.get(item.academic_year_id) || ""}` : ""}</option>)}</select></label>
