...
type SessionsData = {
  ...
  sessionsList: SessionsRecord[];
};

export function SessionsWorkspace() {
  const { data, isLoading } = useSchoolQuery<SessionsData>('/admin-command/guidance-counselling/sessions');
  ...
  return (
    <Panel title="Counselling Sessions" description="Schedule and manage counselling sessions." icon={Calendar}>
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        ...
        <div className="rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-4">
          <div className="text-sm font-semibold text-[#64748B]">Completed This Term</div>
          <div className="mt-1 text-lg font-black text-[#071D49]">{isLoading ? "..." : data?.metrics?.completed_this_term ?? 0}</div>
        </div>
      </div>
