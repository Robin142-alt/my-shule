...
export function UnscheduledLessonsPanel({
  ...
  return (
    <section className="space-y-4 rounded-xl border border-[#D8E0EC] bg-white p-4 sm:p-5" aria-label="Unscheduled lessons">
      ...
      {loading ? <p className="rounded-xl bg-[#F8FAFC] p-6 text-center text-sm font-bold text-[#64748B]">Loading unresolved requirements...</p> : error ? <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-800"><p>Unscheduled requirements could not be loaded.</p><button type="button" onClick={onRetry} className="mt-2 min-h-10 rounded-lg border border-rose-300 bg-white px-3"><RefreshCw className="mr-2 inline h-4 w-4" />Retry</button></div> : items.length === 0 ? <div className="rounded-xl border border-dashed border-[#C8D5EA] bg-[#F8FAFC] p-6 text-center text-sm font-bold text-[#64748B]">No unscheduled lessons have been reported. Generate a timetable or validate the draft to check that all required periods are covered.</div> : (
