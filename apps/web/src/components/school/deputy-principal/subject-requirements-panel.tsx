...
export function SubjectRequirementsPanel({ response, academicYear, termName, classes, subjects, teachers, resources, assignments = [], configuration, onContinue, onDirtyChange, loading, error, onRetry, onSaved, onSaveState }: {
  ...
  const update = (index: number, patch: Partial<TimetableRequirement>) => edit((current) => current.map((item, i) => i === index ? { ...item, ...patch } : item));
  ...
  const addAllocatedSubjects = () => {
    ...
    const candidates = assignments.filter((assignment) => (!classFilter || assignment.class_section_id === classFilter)
      && classes.some((row) => row.id === assignment.class_section_id) && subjects.some((row) => row.id === assignment.subject_id));
    const next = [...items];
    for (const assignment of candidates) {
      if (!next.some((row) => row.class_section_id === assignment.class_section_id && row.subject_id === assignment.subject_id)) {
        next.push({ class_section_id: assignment.class_section_id, subject_id: assignment.subject_id, teacher_id: null, periods_per_week: defaultPeriods, duration_periods: 1 });
      }
    }
  ...
  return <section className="space-y-4" aria-label="Subject period requirements">
    ...
    <fieldset disabled={saveMutation.isPending} className="min-w-0 space-y-4 disabled:opacity-60">
      ...
      {visibleItems.map(({ item, index }) => {
        const allocatedIds = new Set(assignments.filter((row) => row.class_section_id === item.class_section_id && row.subject_id === item.subject_id).map((row) => row.teacher_user_id));
        ...
        return <article key={item.id ?? `new-${index}`} aria-label={`Requirement ${index + 1}`} className="space-y-3 rounded-xl border border-[#D8E0EC] bg-white p-4">
          ...
          <p className="text-sm text-[#64748B]">Teacher: {selectedTeacher ? teacherLabel(selectedTeacher) : allocatedNames.length ? allocatedNames.join(", ") : "No allocation found. Assign a teacher in Academic Setup before generating."}</p>
          <details open={Boolean(item.resource_id || item.parallel_key || item.teacher_id || item.stream_id)}>
            <summary className="cursor-pointer text-sm font-bold text-[#174EA6]">Teacher, room & parallel options</summary>
            <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              <label className="min-w-0 text-sm font-bold">Allocated teacher<select value={item.teacher_id ?? ""} onChange={(event) => update(index, { teacher_id: event.target.value || null })} className={fieldClass}><option value="">Use academic allocation</option>{teachers.map((row) => <option key={teacherId(row)} value={teacherId(row)}>{teacherLabel(row)}</option>)}</select></label>
