...
const controlClass = "mt-1 h-11 w-full min-w-0 rounded-lg border border-[#C8D5EA] bg-white px-3 text-base sm:text-sm font-semibold text-[#071D49] outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-200/30";
...
export function DeputyTimetableManagementWorkspace() {
  ...
  const classStreams = [...new Map(assignments.filter((row) => row.class_section_id === effectiveClassId && row.stream_id && (!row.academic_term_id || row.academic_term_id === selectedTerm?.id)).map((row) => [row.stream_id!, row.stream_name || row.stream_id!])).entries()];
  const effectiveStreamId = classStreams.some(([id]) => id === selectedStreamId) ? selectedStreamId : "";
  ...
  const viewParams = (() => {
    ...
    if (view === "class" && effectiveStreamId) params.set("stream_id", effectiveStreamId);
  ...
  const generate = async () => {
    setSaveState("saving");
    try {
      const result = await generateMutation.mutateAsync({ academic_year: academicYear, term_name: termName, expected_version_row_version: version?.row_version, scope: "whole_school", preserve_locked: true, allow_partial: false });
      if (isOfflineQueued(result)) {
  ...
  const regenerate = async () => {
    ...
    try {
      ...
      const result = await regenerateMutation.mutateAsync({ academic_year: academicYear, term_name: termName, expected_version_row_version: version?.row_version, scope, scope_id: scopeType === "school" ? undefined : scopeId, preserve_locked: true, allow_partial: false, confirm_scope: true });
  ...
  const moveTo = async (slot: TimetableSlot, destination: CandidateSlot, shouldRecord = true) => {
    ...
    try {
      ...
      const result = await postServer<TimetableSlot>(`/api/timetable/slots/${encodeURIComponent(slot.id)}/move`, { day_of_week: destination.day_of_week, period_id: destination.period_id, swap_slot_id: destination.swap_slot_id, expected_row_version: slot.row_version, expected_version_row_version: version?.row_version });
      if (await finishMutation(result, `${slot.subject_name} moved to ${dayLabel(destination.day_of_week)}.`) && shouldRecord) {
        recordOperation({ kind: "move", slotId: slot.id, label: `${slot.subject_name} - ${slot.class_name}`, before: { day_of_week: slot.day_of_week, period_id: slot.period_id ?? "", swap_slot_id: destination.swap_slot_id, state: "VALID", reasons: [] }, after: destination, rowVersion: result.row_version });
      }
      return result;
    } catch (error) {
      setSaveState("failed");
  ...
  const performHistoryOperation = async (direction: "undo" | "redo") => {
    ...
    try {
      ...
      const result = operation.kind === "move"
        ? await postServer<TimetableSlot>(`/api/timetable/slots/${encodeURIComponent(operation.slotId)}/move`, { day_of_week: (target as CandidateSlot).day_of_week, period_id: (target as CandidateSlot).period_id, swap_slot_id: (target as CandidateSlot).swap_slot_id, expected_row_version: currentSlot.row_version, expected_version_row_version: version?.row_version })
    ...
    <div className="space-y-3 rounded-xl border border-[#D8E0EC] bg-[#F8FAFC] p-3">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex flex-1 flex-col gap-3 sm:flex-row">
          ...
          {view === "class" && classStreams.length > 0 ? <label className="max-w-sm flex-1 text-xs font-black uppercase text-[#64748B]">Stream<select value={effectiveStreamId} onChange={(event) => setSelectedStreamId(event.target.value)} className={controlClass}><option value="">All streams</option>{classStreams.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label> : null}
  ...
  return (
    <div className="min-w-0 space-y-4" data-testid="deputy-timetable-management-workspace">
      <section className="rounded-xl border border-[#D8E0EC] bg-white p-4 text-[#071D49] sm:p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div className="min-w-0"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[#47658F]"><CalendarClock className="h-4 w-4" /> Smart timetable & relief</p><h2 className="mt-2 text-xl font-bold sm:text-2xl">Timetable command centre</h2><p className="mt-1 max-w-3xl text-sm text-[#64748B]">Build a balanced week, adjust lessons, then publish.</p></div>
      ...
      <nav className="grid grid-cols-2 gap-1 rounded-xl border border-[#D8E0EC] bg-white p-2 sm:flex sm:flex-wrap sm:gap-2" aria-label="Timetable workspace tabs">
        {([[
      ...
      {(activeTab === "command" || activeTab === "draft") ? <div className="flex flex-col gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-[#071D49] sm:flex-row sm:items-center sm:justify-between"><p className="text-sm leading-relaxed"><strong>A full, balanced teaching week.</strong> Automatic subjects share all available periods. Fixed counts, breaks and locked lessons are respected.</p>{plannerSlots.length > 0 && isEditableDraft ? <button type="button" onClick={() => { if (window.confirm("Rebuild the full draft with balanced subjects and no empty teaching periods? Locked lessons will be kept.")) void generate(); }} disabled={pendingAction} className="min-h-11 shrink-0 rounded-lg bg-[#174EA6] px-4 text-sm font-bold text-white disabled:opacity-50">{generateMutation.isPending ? "Generating..." : "Rebuild & fill week"}</button> : null}</div> : null}
      ...
      <Modal mobileFullScreen open={slotModalOpen} onClose={() => setSlotModalOpen(false)} title={editingSlotId ? "Edit draft lesson" : "Build lesson manually"}>
      ...
      <Modal mobileFullScreen footer={<div className="flex flex-col-reverse gap-2 w-full sm:flex-row sm:justify-end"><button type="button" onClick={() => setPlacementTarget(null)} className="min-h-11 rounded-lg border border-[#C8D5EA] px-4 font-bold">Cancel</button><button type="button" onClick={confirmPlacement} disabled={!selectedCandidate || slotActionPending} className="min-h-11 rounded-lg bg-[#174EA6] px-4 font-black text-white disabled:opacity-50">{slotActionPending ? "Validating & saving..." : selectedCandidate?.swap_slot_id ? "Confirm swap" : "Confirm destination"}</button></div>} open={Boolean(placementTarget)} onClose={() => setPlacementTarget(null)} title={placementTarget?.kind === "slot" ? "Move lesson to a valid period" : "Place unscheduled lesson"}>
        <div className="space-y-4 py-3 text-[#071D49]"><p className="text-sm text-[#64748B]">Choose a new time or swap with an unlocked lesson of the same length. Both teachers, rooms and lesson times are checked before saving.</p><button type="button" onClick={() => placementTarget && openPlacement(placementTarget, true)} disabled={candidateLoading} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-[#174EA6] bg-blue-50 px-4 text-sm font-black text-[#174EA6]"><Sparkles className="h-4 w-4" /> Find Best Slot</button>{candidateLoading ? <div className="rounded-xl bg-[#F8FAFC] p-8 text-center text-sm font-bold text-[#64748B]">Calculating safe candidate periods...</div> : candidateError ? <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-800"><p>{candidateError}</p><button type="button" onClick={() => placementTarget && openPlacement(placementTarget)} className="mt-2 min-h-10 rounded-lg border border-rose-300 bg-white px-3">Retry</button></div> : candidateSlots.length === 0 ? <div className="rounded-xl border border-dashed border-amber-300 bg-amber-50 p-5 text-sm font-bold text-amber-900">No valid destination currently exists. Adjust availability, configuration, locks, or another placement; random invalid periods are not offered.</div> : <div className="space-y-2">{candidateSlots.map((candidate) => { const period = periodFor(configuration, candidate.day_of_week, candidate.period_id); const key = `${candidate.day_of_week}-${candidate.period_id}`; return <label key={key} className={`block cursor-pointer rounded-xl border p-3 ${selectedCandidate?.day_of_week === candidate.day_of_week && selectedCandidate.period_id === candidate.period_id ? "border-[#174EA6] bg-blue-50" : "border-[#D8E0EC] bg-white"}`}><div className="flex items-start gap-3"><input type="radio" name="candidate_slot" checked={selectedCandidate?.day_of_week === candidate.day_of_week && selectedCandidate.period_id === candidate.period_id} onChange={() => setSelectedCandidate(candidate)} className="mt-1" /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-black">{dayLabel(candidate.day_of_week)} - {period?.name ?? candidate.period_id}</span><span className={`rounded-full px-2 py-1 text-[10px] font-black ${candidate.state === "PREFERRED" ? "bg-emerald-100 text-emerald-800" : "bg-blue-100 text-blue-800"}`}>{candidate.swap_slot_id ? "SWAP" : candidate.state}</span></div><p className="mt-1 text-xs text-[#64748B]">{period ? `${timeLabel(period.starts_at)}-${timeLabel(period.ends_at)}` : "Configured period"}</p>{candidate.reasons.length > 0 ? <ul className="mt-2 list-disc pl-4 text-xs text-[#475569]">{candidate.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul> : null}</div></div></label>; })}</div>}</div>
      </Modal>
    </div>
  );
}
