...
export function ResourceRequestsWorkspace() {
  const { data, isLoading } = useSchoolQuery<ResourceRequestsData>("/admin-command/teacher/resource-requests");
  ...
  return (
    <section className="rounded-2xl border border-[#D8E0EC] bg-white p-5 shadow-[0_18px_50px_rgba(7,29,73,0.08)]">
      <div className="mb-4 flex min-w-0 gap-3">
        ...
        <div>
          <h2 className="text-xl font-black tracking-[-0.01em] text-[#071D49]">Resource Requests</h2>
          <p className="mt-1 text-sm leading-6 text-[#64748B]">Request teaching resources from the department.</p>
        </div>
      </div>
